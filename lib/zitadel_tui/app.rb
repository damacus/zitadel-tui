# frozen_string_literal: true

module ZitadelTui
  class App
    def initialize
      @ui = UI.new
      @config = Config.new
      @client = Client.new(config: @config)
      @authenticated = false
    end

    def run
      @ui.clear
      show_welcome

      run_setup_wizard unless @config.configured?
      authenticate
      main_menu
    rescue Interrupt
      @ui.newline
      @ui.info('Goodbye!')
      exit 0
    rescue StandardError => e
      @ui.error("Fatal error: #{e.message}")
      @ui.error(e.backtrace.first(5).join("\n")) if ENV['DEBUG']
      exit 1
    end

    private

    def show_welcome
      @ui.header('Zitadel Administration TUI')
      @ui.info("Version: #{VERSION}")
      @ui.info("Zitadel URL: #{@config.zitadel_url || 'Not configured'}") if @config.configured?
      @ui.newline
    end

    def run_setup_wizard
      @ui.subheader('First-time Setup')
      @ui.info('Welcome! Let\'s configure your Zitadel connection.')
      @ui.newline

      url = @ui.ask('Zitadel URL (e.g., https://zitadel.example.com):') do |q|
        q.required true
        q.validate(%r{\Ahttps?://.+\z}, 'Please enter a valid URL starting with http:// or https://')
      end
      @config.zitadel_url = url

      apps_file = @ui.ask('Apps configuration file path (optional, press Enter to skip):')
      @config.apps_config_file = apps_file unless apps_file.to_s.empty?

      @config.save
      @ui.success('Configuration saved!')
      @ui.newline
    end

    def authenticate
      @ui.subheader('Authentication')

      auth_method = @ui.select_menu('Select authentication method:', [
                                      { name: '🔑 Service Account (JWT)', value: :jwt },
                                      { name: '🎫 Personal Access Token (PAT)', value: :pat }
                                    ])

      case auth_method
      when :jwt
        @ui.spinner('Authenticating with service account...') { @client.authenticate }
      when :pat
        @ui.spinner('Authenticating with PAT...') { @client.authenticate_with_pat }
      end

      @authenticated = true
      @ui.success('Authentication successful!')
      @ui.newline
    rescue AuthenticationError => e
      @ui.error("Authentication failed: #{e.message}")
      retry if @ui.yes?('Try again?')
      exit 1
    end

    def main_menu
      loop do
        @ui.clear
        @ui.header('Zitadel Administration TUI')

        choice = @ui.select_menu('Main Menu - What would you like to manage?', [
                                   { name: '📱 OIDC Applications', value: :apps },
                                   { name: '👥 Users', value: :users },
                                   { name: '🔗 Identity Providers', value: :idp },
                                   { name: '⚙️  Settings', value: :settings },
                                   { name: '🚪 Exit', value: :exit }
                                 ])

        case choice
        when :apps
          Commands::Apps.new(client: @client, ui: @ui).menu
        when :users
          Commands::Users.new(client: @client, ui: @ui).menu
        when :idp
          Commands::Idp.new(client: @client, ui: @ui).menu
        when :settings
          settings_menu
        when :exit
          @ui.info('Goodbye!')
          break
        end
      end
    end

    def settings_menu
      loop do
        @ui.clear
        @ui.header('Settings')

        @ui.info('Current configuration:')
        @ui.info("  Zitadel URL: #{@config.zitadel_url}")
        @ui.info("  Project ID: #{@config.project_id || 'Not set'}")
        @ui.info("  Apps config: #{@config.apps_config_file || 'Not set'}")
        @ui.newline

        choice = @ui.select_menu('Settings', [
                                   { name: '🌐 Change Zitadel URL', value: :url },
                                   { name: '📁 Set Project ID', value: :project },
                                   { name: '📱 Set Apps config file', value: :apps_config },
                                   { name: '💾 Save configuration', value: :save },
                                   { name: '← Back to main menu', value: :back }
                                 ])

        case choice
        when :url
          new_url = @ui.ask('Zitadel URL:', default: @config.zitadel_url)
          @config.zitadel_url = new_url
          @ui.success("URL updated to: #{new_url}")
        when :project
          new_project = @ui.ask('Project ID:')
          @config.project_id = new_project
          @ui.success("Project ID set to: #{new_project}")
        when :apps_config
          new_file = @ui.ask('Apps config file path:', default: @config.apps_config_file)
          @config.apps_config_file = new_file
          @ui.success("Apps config file set to: #{new_file}")
        when :save
          @config.save
          @ui.success('Configuration saved!')
        when :back
          break
        end

        @ui.press_any_key unless choice == :back
      end
    end
  end
end
